...
| `deterministic` | Deterministic | Longest common subsequence of the tokenized golden answer and response, as an F1 measure (ROUGE-L), rounded to two decimal places. Offline, free and bit-deterministic; lexical only, so synonyms and typos score low. Needs a `golden` column. |
...
- `ScoreResult.details` — attach a rationale or per-criterion sub-scores.
  ...
  unrounded, so a reader can see where its two-decimal score came from.
